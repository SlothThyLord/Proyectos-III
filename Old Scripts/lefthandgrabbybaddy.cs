...
public class lefthandgrabbybaddy : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ((OVRInput.Get(OVRInput.Axis1D.PrimaryIndexTrigger,OVRInput.Controller.LTouch) > 0.2f) && CollidingObject)
        {
            GrabObject();
        }

        if ((OVRInput.Get(OVRInput.Axis1D.PrimaryHandTrigger, OVRInput.Controller.LTouch) < 0.2f) && objectInHand)
        {
            ReleaseObject();
        }
    }
